...
// 扫尾、清理函数
void IOCPServer::Clean() {
	CHAR temp[MAX_PATH];

	TRACE("Clean()清理开始。\n");
	if (m_bStart) {
		if (m_hIocp) {
			sprintf(temp, "共有%d个AcceptEx投递失败。\n", m_nFailAccepted);
			TRACE(temp);
...
// 设置监听端口
BOOL IOCPServer::SetPort(int port) {
	BOOL ret = TRUE;
...
// 设置Accept投递函数
BOOL IOCPServer::SetAcceptFun(LPIOCPFUN fun) {
	BOOL ret = TRUE;
...
// 设置Recv投递函数
BOOL IOCPServer::SetRecvFun(LPIOCPFUN fun) {
	BOOL ret = TRUE;
...
// 设置Send投递函数
BOOL IOCPServer::SetSendFun(LPIOCPFUN fun) {
	BOOL ret = TRUE;
...
BOOL IOCPServer::Start() {
	...
	if (!m_bStart) {
		...
		if (NULL == m_hIocp) {
			...
			TRACE("创建m_hIocp完成端口失败！\n");
			goto clean;
		}
		// 创建工作线程
		this->StartWorkerThread(m_nWorkers);
		// 创建清理不活跃连接线程
		...
		if (NULL == m_hCleanSocketThread) {
			TRACE("新建CleanSocketThread线程失败！\n");
		}
		else {
			TRACE("成功新建CleanSocketThread线程。\n");
		}
		// 创建套接字监听端口并绑定到完成端口
		if (!this->SocketListen(m_nPort)) {
			// SocketListen内部已设置m_dwError错误值。
			TRACE("SocketListen()返回失败！\n");
			goto clean;
		}

		// 创建退出工作线程通知事件
		TRACE("创建m_hQuitEvent事件。\n");
		m_hQuitEvent = ::CreateEvent(NULL, TRUE, FALSE, NULL);
		if (NULL == m_hQuitEvent) {
			m_dwError = GetLastError();
			TRACE("创建m_hQuitEvent事件失败！\n");
			goto clean;
		}

		// 创建停止监听通知事件
		TRACE("创建m_hStopListen事件。\n");
		m_hStopListen = ::CreateEvent(NULL, TRUE, FALSE, NULL);
		if (NULL == m_hStopListen) {
			m_dwError = GetLastError();
			TRACE("创建m_hStopListen事件失败！\n");
			goto clean;
		}

		// 创建关闭监听套接字处理完毕通知事件
		TRACE("创建m_hListenClosed事件。\n");
		m_hListenClosed = ::CreateEvent(NULL, TRUE, FALSE, NULL);
		if (NULL == m_hListenClosed) {
			m_dwError = GetLastError();
			TRACE("创建m_hListenClosed事件失败！\n");
			goto clean;
		}

		// 创建退出清理连接线程通知事件
		TRACE("创建m_hQuitCleanThread事件。\n");
		m_hQuitCleanThread = CreateEvent(NULL, TRUE, FALSE, NULL);
		if (NULL == m_hQuitCleanThread) {
			m_dwError = GetLastError();
			TRACE("创建m_hQuitCleanThread事件失败！\n");
			goto clean;
		}
...
// 启动工作线程
void IOCPServer::StartWorkerThread(int num) {
	CHAR temp[MAX_PATH];
	SYSTEM_INFO info;
	UINT dwId;
...
void CleanIoDataQueued(PER_HANDLE_DATA *pHandleData, PER_IO_DATA *pIoData) {
	...
	for (int i = 0; i<(*pHandleData->pAccepted); i++) {
		if (pHandleData->pIoArray[i] == pIoData) {
			...
			sprintf(temp,"成功清理pIoArrary[%d](%08X)。\n", i, (DWORD)pIoData);
			TRACE(temp);
			break;
		}
	}
	LeaveCriticalSection(pHandleData->pcs);
	TRACE("CleanIoDataQueued()结束。\n");
}
...
UINT WINAPI CleanSocketThread(LPVOID param) {
	...
	time_t second;
	CHAR temp[MAX_PATH], szText[MAX_PATH];
	char *szIP;
	int port;
				...
				if (time(NULL) - second > MAX_SECONDS) {
					if (0 == pHandleData->pIoArray[i]->LoginId) {
						...
						if (time(NULL) - second > 60 * 60 * 2) {
							sprintf(temp, "LoginId:%d帐户未活动2小时被关闭。", pHandleData->pIoArray[i]->LoginId);
							TRACE(temp);
							if (!pHandleData->pIoArray[i]->bClose) {
								pHandleData->pIoArray[i]->bClose = TRUE;
								closesocket(pHandleData->pIoArray[i]->socket);
...
UINT WINAPI WorkerThread(LPVOID param) {
	...
	sprintf(temp, "第%d个WorkerThread开始运行。\n", num);
	TRACE(temp);
	while (TRUE) {
		...
		*/
		if (ret) {
			...
			if (pIoData) {
				...
				if (!pIoData->bAccepted) {
					...
					if (1 == pIoData->AcceptedTime) {
						...
						if (INVALID_SOCKET == pNewIoData->socket) {
							TRACE("新建客户端通信socket失败，无法进行AcceptEx投递。(WorkerThread里)\n");
							delete pNewIoData;
						}
						else {
							EnterCriticalSection(pHandle->pcs);
							if (WAIT_OBJECT_0 != WaitForSingleObject(*pHandle->phStopListen, 0)) {
								if (TRUE == ((pHandle->AcceptEx)(*pHandle->pSocket, pNewIoData->socket,
									...
									// AcceptEx投递失败，清除分配内存
									TRACE("AcceptEx投递失败！(WorkerThread里)\n");
								...
								TRACE("检测到停止监听，终止AcceptEx投递！(WorkerThread里)");
								closesocket(pNewIoData->socket);
								delete  pNewIoData;
								(*pHandle->pFailAccepted)++;
								if (MAX_ACCEPT == *pHandle->pFailAccepted) {
				...
				// 1.关闭完成端口句柄
				// 2.其他未知原因意外退出
				sprintf(temp, "GetQueuedCompletionStatus()未知原因错误返回!GetLastError:%d。\n", dwError);
				TRACE(temp);
			}
			continue;
		}
	...
	sprintf(temp, "第%d个WorkerThread结束退出。\n", num);
	TRACE(temp);
	return 0;
}
...
void AcceptHandle(PER_HANDLE_DATA *pHandleData, PER_IO_DATA *pIoData, DWORD dwBytes) {
	...
	pNewIoData->socket = WSASocket(AF_INET, SOCK_STREAM, 0, NULL, 0, WSA_FLAG_OVERLAPPED);
	if (INVALID_SOCKET == pNewIoData->socket) {
		TRACE("新建客户端通信socket失败，无法进行AcceptEx投递。(AcceptHandle里)\n");
		delete pNewIoData;
	}
	else {
		EnterCriticalSection(pHandleData->pcs);
		if (WAIT_OBJECT_0 != WaitForSingleObject(*pHandleData->phStopListen, 0)) {
			if (TRUE == ((pHandleData->AcceptEx)(*pHandleData->pSocket, pNewIoData->socket,
				...
				// AcceptEx投递失败，清除分配内存
				TRACE("AcceptEx投递失败！(AcceptHandle里)\n");
				closesocket(pNewIoData->socket);
				delete  pNewIoData;
			...
			TRACE("检测到停止监听，终止AcceptEx投递！(AcceptHandle里)");
			closesocket(pNewIoData->socket);
			delete  pNewIoData;
			(*pHandleData->pFailAccepted)++;
			if (MAX_ACCEPT == *pHandleData->pFailAccepted) {
...
// 创建套接字监听端口并绑定到完成端口
BOOL IOCPServer::SocketListen(int port) {
	CHAR temp[MAX_PATH];
	BOOL ret = TRUE;
	PER_IO_DATA *pIoData;
	...
	TRACE("SocketListen()开始。\n");
	// TCP流
	TRACE("WSASocket()新建监听套接字。\n");
	m_socket = WSASocket(AF_INET, SOCK_STREAM, 0, NULL, 0, WSA_FLAG_OVERLAPPED);
	if (INVALID_SOCKET == m_socket) {
		m_dwError = WSAGetLastError();
		TRACE("WSASocket()返回失败！\n");
		goto clean;
	}
	...
	TRACE("获取GetAcceptExSockAddr函数指针。\n");
	if (SOCKET_ERROR == WSAIoctl(m_socket, SIO_GET_EXTENSION_FUNCTION_POINTER,
	...
	for (i = 0; i<MAX_ACCEPT; i++) {
		...
		if (INVALID_SOCKET != pIoData->socket) {
			sprintf(temp, "成功新建第%d个客户端套接字。\n", i + 1);
			TRACE(temp);
		}
		else {
			m_dwError = ::WSAGetLastError();
			::sprintf(temp, "新建第%d个客户端套接字失败！\n", i + 1);
			TRACE(temp);
			delete pIoData;
			continue;
		}
